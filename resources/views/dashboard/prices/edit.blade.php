                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-12">
                                            <div class="card">
                                                ...
                                                <form action="{{ route('dashboard.prices.update',$data->id) }}"
                                                      method="post"
                                                                ...
                                                                <select class="form-control" name="zone_id"
                                                                    ...
                                                                    @foreach($zones as $zone)
                                                                        <option value="{{$zone->id}}"
                                                                                @if($data->zone_id==$zone->id)  selected @endif>{{$zone->name}}</option>
                                                                ...
                                                                <select class="form-control" name="block_id"
                                                                    ...
                                                                    @foreach($blocks as $block)
                                                                        <option value="{{$block->id}}"
                                                                                @if($data->block_id==$block->id)  selected @endif>{{$zone->name}}</option>
    ...
    <script>
        ...
        $('#zone_id').on('change', function (e) {
            ...
            $.get("{{url('dashboard/zoneSelectBlock')}}/" + zone_id, function (data) {
                console.log(data);
                $('#block_id').empty();
                $('#block_id').append('<option value="0">@lang('site.select')</option>');
        ...
        $('#block_id').on('change',function(e){
            ...
            $.get("{{url('dashboard/BlockSelectUnit')}}/"+block_id, function(data){
                ...
                $.each(data, function(key, value){
                    var newRow = jQuery('<tr><td><div class="row">  <div class="col-md-5 form-group col-12 p-2"> <label>Units Number</label>' +
                        '<input type="text"  name="unit_number[]"  value="'+value.units_number+'" class="form-control"  readonly /></div><div class="col-md-5 form-group col-12 p-2"> <label>Price</label> ' +
